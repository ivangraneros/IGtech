...
import logo from '../images/logo-IGtech.png'
...
function Navbar({ language, setLanguage, darkMode, setDarkMode }) {
  ...
  const texts = {
    es: { home: "Inicio", services: "Servicios", methodology: "Metodología", projects: "Proyectos", contact: "Contacto" },
    en: { home: "Home", services: "Services", methodology: "Methodology", projects: "Projects", contact: "Contact" }
  };
  ...
  return (
    ...
    <nav className="navbar">
        <div className="navbar-logo">
          <Link to="/">
            <img src={logo} alt="logo igtech" width="200" height="100" />
          </Link>  
        </div>

        <div className={`navbar-links ${isOpen ? 'open' : ''}`}>
          <Link to='/' className="nav-btn-link" onClick={handleLinkClick}>{t.home}</Link>
          <Link to='/servicios' className="nav-btn-link" onClick={handleLinkClick}>{t.services}</Link>
          <Link to='/metodologia' className="nav-btn-link" onClick={handleLinkClick}>{t.methodology}</Link>
          <Link to='/contacto' className="nav-btn-link" onClick={handleLinkClick}>{t.contact}</Link>
      ...
      {isMobile && (
        <label class="hamburger">
          <input 
          type="checkbox" 
          className={`hamburguer-btn ${isOpen ? "active" : ''}`} 
